...
fn calc_green_and_yellow(guess: Vec<u8>, secret: &[u8]) -> String {
    assert!(
...
fn test_green_and_yellow() {
    assert_eq!(
        calc_green_and_yellow([1, 2, 3, 4].to_vec(), &[1, 2, 3, 4]),
        "🟩 🟩 🟩 🟩".to_string()
    );
    assert_eq!(
        calc_green_and_yellow([1, 2, 3, 5].to_vec(), &[1, 2, 3, 4]),
        "🟩 🟩 🟩 ⬜".to_string()
    );
    assert_eq!(
        calc_green_and_yellow([4, 3, 2, 1].to_vec(), &[1, 2, 3, 4]),
        "🟨 🟨 🟨 🟨".to_string()
    );
    assert_eq!(
        calc_green_and_yellow([1, 2, 3, 1].to_vec(), &[1, 2, 3, 4]),
        "🟩 🟩 🟩 ⬜".to_string()
    );
    assert_eq!(
        calc_green_and_yellow([1, 1, 1, 1].to_vec(), &[1, 2, 3, 4]),
        "🟩 ⬜ ⬜ ⬜".to_string()
    );
    assert_eq!(
        calc_green_and_yellow([1, 2, 2, 2].to_vec(), &[2, 2, 2, 1]),
        "🟨 🟩 🟩 🟨".to_string()
    );
    assert_eq!(
        calc_green_and_yellow([1, 3, 3, 2].to_vec(), &[2, 2, 2, 1]),
        "🟨 ⬜ ⬜ 🟨".to_string()
    );
}
...
fn main() {
    ...
    loop {
        ...
        let squares = calc_green_and_yellow(guess.clone(), &secret);
